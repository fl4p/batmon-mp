...
def test_varint_coding():
    ...
    def decodeVarintBuf(clear=True):
        v, p = SignedVarintDecode(buf, 0)
        assert p == len(buf)
        if clear:
            buf.clear()
        return v

    assert (SignedVarintEncode(_write, ZigZagEncode(63)), len(buf)) == (1, 1), len(buf)
    assert ZigZagDecode(decodeVarintBuf()) == 63
    assert (SignedVarintEncode(_write, ZigZagEncode(64)), len(buf)) == (1,2), len(buf)
    assert ZigZagDecode(decodeVarintBuf()) == 64
    assert (SignedVarintEncode(_write, ZigZagEncode(127)), len(buf)) == (1,2), len(buf)
    assert ZigZagDecode(decodeVarintBuf()) == 127
    assert (SignedVarintEncode(_write, ZigZagEncode(128)), len(buf)) == (1,2), len(buf)
    assert ZigZagDecode(decodeVarintBuf()) == 128
    assert (SignedVarintEncode(_write, ZigZagEncode(256)), len(buf)) == (1,2), len(buf)
    assert ZigZagDecode(decodeVarintBuf()) == 256
    assert (SignedVarintEncode(_write, ZigZagEncode(1024)), len(buf)) == (1,2), len(buf)
    assert ZigZagDecode(decodeVarintBuf()) == 1024

    assert (SignedVarintEncode(_write, ZigZagEncode(-63)), len(buf)) == (1, 1), len(buf)
    assert ZigZagDecode(decodeVarintBuf()) == -63
    assert (SignedVarintEncode(_write, ZigZagEncode(-64)), len(buf)) == (1, 1), len(buf)
    assert ZigZagDecode(decodeVarintBuf()) == -64
    assert (SignedVarintEncode(_write, ZigZagEncode(-65)), len(buf)) == (1, 2), len(buf)
    assert ZigZagDecode(decodeVarintBuf()) == -65

    assert (SignedVarintEncode(_write, 64), len(buf)) == (1, 1), len(buf)
    assert decodeVarintBuf() == 64
    assert (SignedVarintEncode(_write, 127), len(buf)) == (1, 1), len(buf)
    assert decodeVarintBuf() == 127
    assert (SignedVarintEncode(_write, 128), len(buf)) == (1, 2), len(buf)
    assert decodeVarintBuf() == 128
...
def test_shard_store():
    ...
    datasets = [
        ...
        # [dict(time=(i * 100) % 65535, # monotonic = False
        #      voltage=int((12 + math.sin(i / 5)) * 100),
        #      current=int(math.sin(i / 5) * 100),
        #      soc2=int(abs(math.sin(i / 50)) * 100 * 2),
        #      cell_min=3340 + int(math.sin(i / 5) * 20),

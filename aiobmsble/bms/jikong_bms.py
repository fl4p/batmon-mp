...
class BMS(BaseBMS):
    ...
    def _notification_handler(
        ...
        self._data_final = bytearray(self._data)
    ...
    async def _init_connection(
        ...
        if char_notify_handle == -1 or self._char_write_handle == -1:
            ...
            raise Exception(f"Failed to detect characteristics from {self.name}.")
        self._log.debug(
    ...
    def _cmd(cmd: bytes, value: list[int] | None = None) -> bytes:
        ...
        assert value is None or len(value) <= 13
        frame: bytearray = bytearray(len(BMS.HEAD_CMD) + 1 + 1 + 13)
        frame[0:len(BMS.HEAD_CMD)] = BMS.HEAD_CMD
        frame[len(BMS.HEAD_CMD)] = cmd[0]
        frame[len(BMS.HEAD_CMD)+1] = len(value) if value else 0
        if value:
            frame[len(BMS.HEAD_CMD) + 2:] = value
    ...
    def _dec_devinfo(data: bytearray) -> dict[str, str]:
        ...
        return {
            key: data[idx : idx + 8].decode().strip("\x00")

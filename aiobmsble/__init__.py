...
class BMSsample(dict):
    pass

class BMSsample_(dict):
...
class BMSdp():
    def __init__(self, key: BMSvalue, pos: int, size: int, signed: bool, fct: Callable[[int], Any] = lambda x: x,
                 idx=-1):
        self.key = key
        self.pos = pos
        self.size = size
...
class MatcherPattern(TypedDict):
